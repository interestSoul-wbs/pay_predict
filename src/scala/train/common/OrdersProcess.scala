...
import mam.Dic
import mam.Utils
import mam.Utils.{printDf, saveProcessedData, udfChangeDateFormat, udfFillDiscountDescription, udfGetDays, udfGetErrorMoneySign, udfGetKeepSign, udfUniformTimeValidity}
...
object OrdersProcess {
  ...
  def main(args: Array[String]): Unit ={
    ...
    saveProcessedData(orderProcessed, orderProcessedPath)
    println("订单数据处理完成！")
  }
  ...
  def orderProcess(orderRaw:DataFrame)={
    ...
    orderProcessed = orderProcessed.dropDuplicates().na.drop("all")
      //计算有效时长
