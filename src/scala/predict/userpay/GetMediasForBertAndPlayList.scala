...
object GetMediasForBertAndPlayList {
  ...
  def main(args: Array[String]): Unit = {
    ...
    getMediasForBertAndPlayList(df_plays, df_medias, df_predict_ids, playsNum, predictTime)

  }

    def getMediasForBertAndPlayList(df_play: DataFrame, df_medias: DataFrame, df_predict_ids: DataFrame, playsNum: Int, predictTime: String) = {

      // 一段时间的播放历史
      val df_plays_part = df_play.filter(
        col(Dic.colPlayStartTime) < (predictTime)
          && col(Dic.colPlayStartTime) >= calDate(predictTime, -days)
      )

      // 训练集用户的播放历史
      val df_predict_plays = df_plays_part.join(df_predict_ids, Seq(Dic.colUserId), "inner")
      ...
      val df_play_list = df_predict_ids
