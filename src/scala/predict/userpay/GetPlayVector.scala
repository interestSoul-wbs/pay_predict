...
object GetPlayVector {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val df_medias_vector = getDataFromXXK("predict", "predict_medias_bert_w2v_vec")
    printDf("Input: df_medias_vector", df_medias_vector)

    // 3 Get play history in a past period of time

    val df_predict_play = getDataFromXXK("predict", "predict_play_free_paid_" + playsNum)

    printDf("输出 df_predict_play", df_predict_play)
  ...
  def mapId2Vec2(df_medias_vector: DataFrame, df_predict_play: DataFrame) = {


    var df_play_his = df_predict_play
