...
object OrderAndPlayVectorProcess {
  ...
  def main(args: Array[String]): Unit = {
    ...
    printDf("df_predict_ser", df_predict_user)
    ...
    printDf("df_order_history in past three months", df_order_history)
    ...
    printDf("df_predict_play_history", df_predict_play_history)
    ...
    //    printDf("df_predict_play_Vector", df_predict_play_Vector)
    ...
    val df_video_times_vector = df_video_vector.join(df_video_played_times, Seq(Dic.colVideoId), "inner")
  ...
  def getOrderHistoryList(df_predict_id: DataFrame, df_orders: DataFrame, now: String, timeLength: Int) = {
     ...
     */
    val df_order_part = df_orders.join(df_predict_id, Seq(Dic.colUserId), "inner")
  ...
  def getVideoPlaysTimes(df_plays: DataFrame, predict_time: String, timeLength: Int, df_medias: DataFrame) = {
    ...
    val df_predict_play = df_plays.filter(col(Dic.colPlayStartTime).<(predict_time) and col(Dic.colPlayStartTime) >= calDate(predict_time, days = -timeLength))

    //video in package that need to predict
    val df_package_video = df_medias.filter(col(Dic.colPackageId) === 100201 or col(Dic.colPackageId) === 100202)
    ...
    df_package_play_history.groupBy(Dic.colVideoId).agg(count(Dic.colPlayStartTime).as(Dic.colPlayTimes))
  }
  ...
  def getPlaySeqList(df_predict_id: DataFrame, df_play: DataFrame, colName: String, now: String, timeWindowPlay: Int, topNPlay: Int) = {
    ...
    val df_play_list = df_play.join(df_predict_id, Seq(Dic.colUserId), "inner")
     ...
     */
    df_play_list.withColumn("index", row_number().over(win))
      .withColumn("0", lit("0")) //要pad的字符
      .withColumn("tmp_rank", udfLpad(col("index"), lit(rowCount), col("0"))) //拼接列
      .drop("0")
  ...
  def mediasVectorProcess(df_medias_vector_v1: DataFrame, predictTime: String) = {
    ...
    val df_medias_vector = df_medias_vector_v1.withColumn("predictTime", lit(predictTime))
  ...
  def fillStorageGap(df_medias: DataFrame): DataFrame = {
    ...
    df_medias_mean.withColumn(Dic.colStorageTimeGap, when(col(Dic.colStorageTimeGap).>=(0.0), col(Dic.colStorageTimeGap))
  ...
  def mapVideoVector(df_predict_play_history: DataFrame, df_video_vector: DataFrame, topNPlayHistory: Int) = {
    ...
    val df_play_vector = df_predict_play_history.withColumn("play_vector", mapIdToMediasVector(mediasMap)(col(Dic.colVideoId + "_list")))
